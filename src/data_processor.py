...
# Custom processor cho dữ liệu text/label và text/pseudo_label
class CustomDatasetProcessor(BaseDatasetProcessor, GenericLabelProcessor):
    def __init__(self, label_map=None, pseudo_label_col=None):
        self.label_map = {} if label_map is None else label_map
        self.pseudo_label_col = pseudo_label_col  # Nếu là unlabel thì truyền 'pseudo_label'

    def process_dataframe(self, dataframe):
        # Nếu là file unlabel thì dùng pseudo_label, còn lại dùng label
        label_col = self.pseudo_label_col if self.pseudo_label_col else 'label'
        ...
        dataframe['label'] = dataframe[label_col].apply(lambda l: self.get_numeric_label(l, self.label_map))
        dataframe = dataframe[['id', 'text', 'label']]
        return dataframe
import torch
...
class CustomDatasetProcessor:
    ...
    def process_dataframe(self, dataframe):
        ...
        label_col = self.pseudo_label_col if self.pseudo_label_col else 'label'
        dataframe['id'] = dataframe.get('id') if 'id' in dataframe.columns else dataframe.index
        # extract_int_from_string: dùng lại logic từ BaseDatasetProcessor
        def extract_int_from_string(s):
            ...
            return None
        dataframe['id'] = dataframe['id'].apply(extract_int_from_string)
        # get_numeric_label: dùng lại logic từ GenericLabelProcessor
        def get_numeric_label(label, label_map):
            if isinstance(label, int) and label in label_map.values():
                return label
            elif isinstance(label, str) and label.isdigit():
                return int(label)
            return label_map.get(label, -1)
        dataframe['label'] = dataframe[label_col].apply(lambda l: get_numeric_label(l, self.label_map))
        dataframe = dataframe[['id', 'text', 'label']]
        return dataframe
