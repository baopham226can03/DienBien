...
from src.data_processor import CustomDatasetProcessor, TextDataset
...
# Hàm tạo dataloader cho custom dataset text/label hoặc text/pseudo_label
def create_custom_dataloader(dataframe, tokenizer, batch_size, max_len, label_map=None, pseudo_label_col=None):
    processor = CustomDatasetProcessor(label_map=label_map, pseudo_label_col=pseudo_label_col)
    processed_df = processor.process_dataframe(dataframe)
    dataset_obj = TextDataset(processed_df, tokenizer, max_len, dataset=None)
...
def main():
    ...
    # Create dataloaders
    dataloaders = {
        'train_dataloader_1': create_custom_dataloader(trainingSet_1, tokenizer, BATCH_SIZE, MAX_LEN),
        'train_dataloader_2': create_custom_dataloader(trainingSet_2, tokenizer, BATCH_SIZE, MAX_LEN),
        'val_dataloader': create_custom_dataloader(validationSet, tokenizer, BATCH_SIZE, MAX_LEN),
        'test_dataloader': create_custom_dataloader(testingSet, tokenizer, BATCH_SIZE, MAX_LEN),
        'auto_label_dataloader': create_custom_dataloader(auto_labeled_data, tokenizer, BATCH_SIZE, MAX_LEN)
    }
    ...
    dataloaders['init_df_dataloader'] = create_custom_dataloader(init_df, tokenizer, BATCH_SIZE, MAX_LEN)
    ...
    # ==== ĐOẠN CODE ĐƠN GIẢN HÓA CHO DATASET 2 LỚP ====
    import pandas as pd
    # Đường dẫn file, bạn có thể chỉnh lại nếu cần
    df_label = pd.read_csv('label.csv')
    df_val = pd.read_csv('val.csv')
    df_test = pd.read_csv('test.csv')
    df_unlabel = pd.read_csv('unlabel.csv')

    # Khởi tạo tokenizer (giữ nguyên logic cũ)
    # ...existing code...

    # Tạo dataloader
    BATCH_SIZE = 32
    MAX_LEN = 300
    train_loader = create_custom_dataloader(df_label, tokenizer, BATCH_SIZE, MAX_LEN)
    val_loader = create_custom_dataloader(df_val, tokenizer, BATCH_SIZE, MAX_LEN)
    test_loader = create_custom_dataloader(df_test, tokenizer, BATCH_SIZE, MAX_LEN)
    unlabel_loader = create_custom_dataloader(df_unlabel, tokenizer, BATCH_SIZE, MAX_LEN, pseudo_label_col='pseudo_label')

    # Khởi tạo model 2 lớp
    from src.models import RoBERTa
    model = RoBERTa(num_classes=2, args=args)

    # Huấn luyện, đánh giá, inference... (bạn có thể viết tiếp tuỳ ý)
    # ...existing code...
